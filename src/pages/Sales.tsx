...
import { useOrdersQuery, OrderFilters, PedidoListItem } from '@/hooks/use-orders-query';
...
import {
  ...
  RMA_STATUS_LABELS, RMA_STATUS_COLORS, PaymentMethod,
...
export { pedidoListToOrder } from '@/lib/order-adapter';
...
export default function Sales() {
  ...
  const openProductModal = (orderId: string, itemId: string) => {
    ...
    const subPurchases: SubPurchase[] = produto.sub_compras && produto.sub_compras.length > 0
      ? produto.sub_compras.map(sp => ({ ...sp, status: sp.status as ItemStatus, paymentMethod: sp.paymentMethod as PaymentMethod | '' }))
                      ...
                      <Table style={{ minWidth: 720 }}>
                        ...
                        <TableBody>
                          {isLoading ? (
                            ...
                            const orderValue = Number(item.valor_venda) || 0;
